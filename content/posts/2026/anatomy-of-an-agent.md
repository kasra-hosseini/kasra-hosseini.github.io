...
This has already reached physical results. DeepMind's GNoME predicted millions of candidate crystal structures and which of them should be stable. In a companion result, Berkeley's A-Lab ran its own experimental loop for seventeen days and synthesized thirty-six of fifty-seven computationally predicted compounds, with humans choosing the targets and handling breakdowns but not directing individual experiments. (Whether all thirty-six were genuinely new is contested; crystallographers questioned some of the phase identifications, and the paper itself later flagged four as inconclusive. The loop ran the equipment either way.) Something similar happens in chemistry with <a href="https://arxiv.org/abs/2304.05376" target="_blank" rel="noopener">ChemCrow</a>, and furthest of all with <a href="https://arxiv.org/abs/2408.06292" target="_blank" rel="noopener">The AI Scientist</a>, which generates its own ideas, runs the experiments, writes them up, and then reviews its own drafts.
...
One line is worth carrying away from all of this. A loop cannot audit itself: an agent left alone with its own judgement does not converge on the truth, it converges on whatever it already believed, only louder. Everything else in this series follows from trying to fix that, and from what the fix costs. If you want the argument for why agents end up in groups at all, that is [Part 1](/posts/2026/agent-fabric-part1/).
