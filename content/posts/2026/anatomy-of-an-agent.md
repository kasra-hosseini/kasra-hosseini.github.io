...
None of this is a shortfall in the model. It starts with the fact that the agent's own output becomes its input next time round: the failures it reads are the ones its own last edit produced, so it is not solving a problem so much as negotiating with a situation it keeps changing. And the errors multiply rather than add. A wrong turn at step four narrows what step five can even attempt, so a chain of nearly-right steps comes apart more often than it holds. Which is why agents still fall short of people on <a href="https://arxiv.org/abs/2404.07972" target="_blank" rel="noopener">realistic computer and web tasks</a>, and why the <a href="https://arxiv.org/abs/2406.12045" target="_blank" rel="noopener">gap widens as the tasks get longer</a>.
