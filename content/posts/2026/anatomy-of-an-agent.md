...
This pattern has already been pushed past software. In materials science and chemistry, loops like this one propose candidate compounds, run the equipment that tries to make them, read the results, and pick what to try next, with humans setting the targets rather than directing each experiment (<a href="https://arxiv.org/abs/2304.05376" target="_blank" rel="noopener">ChemCrow</a>). How much of what comes out is genuinely new is argued over case by case, which is what you would expect this early. The furthest version is <a href="https://arxiv.org/abs/2408.06292" target="_blank" rel="noopener">The AI Scientist</a>, which generates its own ideas, runs the experiments, writes them up, and then reviews its own drafts.
