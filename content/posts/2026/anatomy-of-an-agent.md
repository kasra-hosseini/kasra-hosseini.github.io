...
Start with the acting, because some actions cannot be taken back and that matters more than how clever the agent is. A deleted file comes back from backup. A robot arm that has already swung cannot be unswung. Between those two extremes sit the ordinary cases: reading files is safe, calling a well-behaved service is safe, driving a browser is messy because pages change underneath you. The same loop with the same model is a different proposition depending on which of those it can reach.
