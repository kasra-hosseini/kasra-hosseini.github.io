...
The tightest version happens inside a single task, where a coding agent runs the tests, reads the failures, and fixes the code, so every edit knows what the last one broke. Widen the window and something stranger happens: the agent writes itself a note after a failed attempt, and consults that note next time. Fifty tasks later it is working from a file of things it learned the hard way, and the version of itself that started has effectively been replaced. <a href="https://arxiv.org/abs/2303.11366" target="_blank" rel="noopener">Reflexion</a> works exactly this way, keeping the agent's own critiques of its past attempts. Push it further and the notes stop being notes: procedures that keep working become tools the agent can call, so what it figured out in hour one is still paying off in hour fifty (<a href="https://arxiv.org/abs/2305.16291" target="_blank" rel="noopener">Voyager</a>).

This is not confined to software. The same loop has been pointed at chemistry, where it proposes a compound, reads back what the equipment found, and picks the next thing to try (<a href="https://arxiv.org/abs/2304.05376" target="_blank" rel="noopener">ChemCrow</a>), and at research itself, where <a href="https://arxiv.org/abs/2408.06292" target="_blank" rel="noopener">one system</a> generates its own ideas, runs the experiments, writes them up, and reviews its own drafts. Worth being precise about those, because most are not one agent improving alone: a model proposes, and some later stage, human or automated, throws nearly all of it out. The survivors go into the world, and what happens there feeds the next round.
...
This is not one system's quirk. Models asked to revise their reasoning without outside feedback often come out worse than before (<a href="https://arxiv.org/abs/2310.01798" target="_blank" rel="noopener">Huang et al., 2023</a>), and a survey looking for reliable self-correction found no case of it that did not rely on some signal from outside (<a href="https://arxiv.org/abs/2406.01297" target="_blank" rel="noopener">Kamoi et al., 2024</a>). The mechanism meant to catch mistakes ends up laundering them into certainty.
