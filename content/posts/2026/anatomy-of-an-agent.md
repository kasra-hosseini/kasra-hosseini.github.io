...
Those ordinary failures are worth a moment because they show what the pieces do. An agent with no prompt does something competent that nobody asked for. An agent with no planning fixes the thing in front of it and deletes the file it will need in twenty minutes. Both are visible within the hour, which is exactly what the self-grading failure is not.
