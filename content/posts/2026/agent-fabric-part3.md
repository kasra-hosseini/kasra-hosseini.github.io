...
The tractable ones are still genuinely difficult, and they mostly come back to the same root: almost every tool we have for holding systems accountable assumes the thing being held accountable stays put. Identity is the clearest case. Every authentication scheme ever built assumes a persistent subject, whereas an agent can be cloned, merged, or rolled back to last Tuesday before lunch, and none of those operations has an obvious answer to the question of who is now responsible for what the original did. Provenance fails for a closely related reason, because an agent working inside several societies at once leaves a trail that snaps at every boundary it crosses, so tracing which society's rules produced a given output becomes guesswork.

Then there is the problem of systems that rewrite their own rules under pressure, which human institutions do constantly: democracies reach for emergency powers, markets simply stop trading. Deciding what should trigger such a switch is hard enough. Making sure the emergency mode ever ends is the part nobody solves.
