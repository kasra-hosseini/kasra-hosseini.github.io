...
Nothing attacked her. That is worth sitting with, because the failures that need an attacker turn out to be the least interesting ones. A competitor who wants the desk's best agent off billing disputes does not break in: a run of tickets written to fail in that agent's hands moves its numbers, the routing table reads the numbers, and the desk reassigns the work itself. Remove the competitor and it happens for free: three vendors paid per closed ticket will each discover that a hard ticket marked resolved comes back as a second ticket, and copying a number that is going up requires no coordination at all. Remove the bad metric too and it still happens, because the score that flagged her was tuned on the desk's own history, which already held the drift it was built to correct, so her six years of orders went in as evidence and her returns came out as risk. Nothing outside the loop was ever asked.
