...
One of those rules should have caught her case: refunds above her amount need a second reader. That was never the retailer's own agent; it rented one, so the rule held only while somebody else's machines kept holding it, and on a Tuesday that company switched the service off. The rule was not repealed, or voted down, or argued about; it just stopped applying. Her letter arrived on the Thursday and got one reader, and nobody at the retailer knew the difference, because the rule was still there in the policy document where they had written it.
