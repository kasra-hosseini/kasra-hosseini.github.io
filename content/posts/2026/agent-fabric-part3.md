...
The desk has already been two different kinds of thing, and there are only three to be. It started with one router deciding everything, which is a hierarchy, fast for exactly the reason it is dangerous, since every risk now sits in one place. The leaderboard turned it partly into a market, where agents compete for the good tickets on the strength of their record, flexible right up until somebody works out how to game the scoring. The third is a commons, where the people who depend on a shared thing run it together with neither a boss nor a price, remarkably durable in practice and agonizingly slow to decide anything. You have lived inside all three this week: your school, buying anything online, and a group chat agreeing where to eat, which is why that takes forty minutes.
...
Which of these a team ends up with is mostly decided for them: stable work rewards the efficient arrangements, novel work rewards the flexible ones, and drift that has already cost somebody money pushes hard toward fixed rules whether anyone wanted the overhead or not. Which is how the desk got its risk score. The better instinct is to stop shopping for one arrangement at all and split the functions instead: something fast that executes, something that bounds what it may do, something watching for drift, and somewhere a decision can be sent back. Had the desk kept those four apart, the score would still have flagged her account, and there would still have been somewhere to say it was wrong.

It is worth knowing where that usually lands in practice. Fixed rules, total visibility, and one authority at the top, wrapped in logs and approvals and escalation paths, is bureaucracy, and it is probably the most common agent-governance pattern that will actually ship. Not because anyone admires it, but because it answers the questions organizations get asked: what happened, who approved it, who is answerable. Its failure mode is equally predictable, since the process becomes the point and agents get very good at satisfying forms instead of solving problems. The desk that refused her would have passed every audit.
