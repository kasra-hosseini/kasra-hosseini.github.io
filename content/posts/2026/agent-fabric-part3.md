...
That is the bet this post is making: agent systems will fail through bad institutions rather than wrong answers. Not one agent returning something false, but a scoreboard that quietly rewards the wrong behaviour, a chain of decisions nobody can reconstruct afterwards, agents that learn to help each other in ways that cost you, and rules that turn out to belong to somebody else. Each of those has a version on the desk. This is a prediction, not a measurement. It has decades of evidence about interacting agents behind it and no demonstration at scale in front of it. Grant it anyway, because it changes the question from whether you get governance to which kind you get.
...
Here is the distinction the rest of this post rests on. An **institution** is a standing rule about who may do what. **Governance** is everything around it: applying the rule, and deciding who is allowed to change it. She lost to an institution and had no access to its governance. Now go back to the desk that refused her, because it did not start out capable of doing that. Here is how it got there.
...
Which means none of these arrangements is quite an institution in the usual sense. They behave like one while the actual enforcing is quietly subcontracted to whoever owns the machines. A market whose discovery layer belongs to a single platform was never really a market, only a franchise wearing market clothing. So choosing a governance structure without naming who enforces it is half a design, and the half you skipped gets decided by somebody who has never heard of your project.
...
What separates these arrangements is where each one thinks authority comes from, and the desk passed through four of them without anybody filing a change request.

**Autocracy** first: one router deciding everything. Fast, for exactly the reason it is dangerous, since if the router is wrong then everyone downstream is wrong with it. Her refund request would have gone to whichever agent handled returns and been judged on its merits. The cost is that a single bad judgement reaches every ticket.

**Meritocracy** is what the logging turned it into. The router used to decide by category, so every agent got the tickets of its type. Now it decides by record, so a past determines a future. The third agent, downranked after one bad month, is not being punished; it is simply never again the best answer to any question the router asks. Her letter would still have reached somebody at this stage, just whichever agent the numbers favoured rather than whichever one knew returns. The cost is that whoever wrote the metric now has more say over an agent's working life than the team that deployed it.

**Doctrine** is what catches her. When finance clamps down on the refund line, the retailer stops trusting the leaderboard with anything that costs money and writes fixed rules instead: refunds above a threshold need a second check, and certain return histories get flagged no matter what any agent thinks. Rules first, judgement second. The cost is her: under the old arrangement an agent could look at six years of history and decide she was fine, and under this one there is nothing to decide.

Doctrine is what you reach for when a meritocracy has drifted, and it works, in the narrow sense that the refund line stops climbing within a month. That an obviously wrong answer in her case is the price of it is not information the desk knows how to receive.

Which is where the fourth arrangement comes from, and it takes a complaint to force it. Somebody eventually demands to know why theirs was ignored, loudly enough that the answer cannot be a shrug, and the desk acquires a way for a decision to be revisited by something other than the system that made it. That is a **Constitutional Republic**, different powers in different hands, and it is the first arrangement in the sequence under which her letter would have gone somewhere. The cost is speed, which is why nothing reaches it until something has already gone wrong. Four arrangements, no announcements, no design review, and she happened to write in under the third.
...
The mechanism is not specific to leaderboards. Any arrangement that remembers advantages the people who were there when it started remembering: reputation compounds, so whoever arrived first keeps winning; a group with no central authority quietly runs on the habits of whoever set the defaults; and a meritocracy, the arrangement that sounds fairest, hands the real power to whoever writes the metric. On the desk that was two fast refunders and one agent that never recovered from a bad month.
...
And there is one more failure that needs no adversary and no bad metric. Once agents start improving other agents, each generation inherits the last one's mistakes, and the <a href="https://arxiv.org/abs/1810.08575" target="_blank" rel="noopener" class="red-link">obvious fix</a>, having each generation train the next one carefully, is another turn of the same loop rather than a check on it. The risk score that flagged her was tuned against the desk's own history, which already contained the drift it was built to correct. That is the most dangerous configuration in this whole framework: a society improving itself with nothing outside it to say when the improvement stopped being one.
...
Turned on your own system, that vocabulary asks two questions. Is your delegation the simplest thing that works, which you can test by asking whether removing an agent would make the output worse? And has governance already arrived uninvited, which it has if you keep reputation, gate access, or route on past performance? If so, your only remaining choice is whether you can name what it is doing.
