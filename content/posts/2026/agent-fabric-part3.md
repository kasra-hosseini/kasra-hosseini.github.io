...
This even catches the techniques meant to prevent it. There is a <a href="https://arxiv.org/abs/1810.08575" target="_blank" rel="noopener" class="red-link">proposal for keeping powerful models aligned</a> that has a model train its own replacement, over and over, on the theory that careful supervision at each step carries forward. But stacking rounds is exactly how it builds capability, so it inherits the same compounding by construction. Nothing that improves across generations escapes this.
