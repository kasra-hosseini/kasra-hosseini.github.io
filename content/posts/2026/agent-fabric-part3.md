...
What separates these arrangements is where each one thinks authority comes from, and the desk passed through four of them without anybody filing a change request. Worth tracking one thing as it does: what would have happened to that letter under each.

**Autocracy** first: one router deciding everything. The woman with the refused refund would have had her request go to whichever agent handled returns and been judged on its merits, which from the outside feels like dealing with a small shop where one person decides and you can tell who it was. Fast, for exactly the reason it is dangerous, since if that one judgement is wrong then everyone downstream is wrong with it.

**Meritocracy** is what the logging turned it into. The router used to decide by category, so every agent got the tickets of its type. Now it decides by record, so a past determines a future. Her letter, if she had written it now, would still reach somebody, just whichever agent the numbers favour rather than whichever one knows returns, and from the outside it feels like being routed to whoever is currently winning rather than whoever understands your problem. The third agent, downranked after one bad month, is not being punished; it is simply never again the best answer to any question the router asks, and whoever wrote the metric now has more say over its working life than the team that deployed it.

**Doctrine** is the one that catches the woman from the opening. When finance clamps down on the refund line, the retailer stops trusting the leaderboard with anything that costs money and writes fixed rules instead: refunds above a threshold need a second check, and certain return histories get flagged no matter what any agent thinks. Rules first, judgement second. From the outside this is the one that feels like talking to a wall, because every reply is correct and none of them is about you.
...
Which is where the fourth arrangement comes from, and it takes a complaint to force it. Somebody eventually demands to know why theirs was ignored, loudly enough that the answer cannot be a shrug, and the desk acquires a way for a decision to be revisited by something other than the system that made it. That is a **Constitutional Republic**: different powers in different hands, an appeal that does not report to the thing being appealed. From the outside it is the first arrangement that feels like being heard rather than processed, and the first under which the refused refund would have gone somewhere other than back to the rule that refused it. The cost is speed, which is why nothing reaches it until something has already gone wrong. Four arrangements, no announcements, no design review, and she happened to write in under the third.
...
Turned on your own system, the vocabulary does two things. It tells you whether your delegation is the simplest thing that works, which you can check by removing an agent and seeing whether the output gets worse. And it tells you whether governance has already arrived uninvited, which it has if you keep reputation, gate access, or route on past performance. If it has, your only remaining choice is whether you can name what it is doing, which is the difference between the retailer that could have described what it was adding and the one that could not.
