...
Twenty-two archetypes follow, some already running in production and some only plausible given where things are heading. Comparing them means asking the same four questions each time. How much useful work do you get for what it costs? How well does it stop goals from quietly wandering? Will the participants treat its decisions as binding? And can a human afterwards reconstruct what happened and why? The cards below label those four *efficiency*, *drift resistance*, *legitimacy*, and *legibility*.

One of those labels needs a warning, because it is borrowed and it does not fit perfectly. **Legitimacy**, in political philosophy, has to do with the governed consenting to be governed. Agents cannot consent to anything. So every time the word appears below, read it in the narrow engineering sense: will the participants, and the humans answerable for them, treat this structure's decisions as binding and worth defending? That is a real property with real consequences, and it is not the same property philosophers argue about. Open Problems returns to what gets lost in the substitution.
