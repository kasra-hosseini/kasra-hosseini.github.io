...
That is the bet this post is making: agent systems will fail through bad institutions rather than wrong answers. Not one agent returning something false, but a scoreboard that quietly rewards the wrong behaviour, a chain of decisions nobody can reconstruct afterwards, agents that learn to help each other in ways that cost you, and rules that turn out to belong to somebody else. Each of those has a version on the desk. Worth being straight that this is a prediction rather than a measurement, well motivated by decades of evidence about interacting agents under pressure and not yet demonstrated at scale. Grant it anyway, because it makes the interesting question not whether you get governance but which kind.
...
Now go back to the woman whose refund was refused, because the desk that refused her did not start out capable of it. Here is how it got there.
...
Two words follow from that, and the difference matters for her specifically. The **institution** is the thing that exists: the standing rule that flagged accounts like hers stay flagged. **Governance** is what happens around it, the business of applying that rule and settling who is allowed to change it. She lost to the first and had no access to the second. The desk acquired an institution the month it started keeping score, and it has been governed ever since by nobody in particular.

Memory alone would not do it, though. A cache remembers and governs nothing; what makes the desk's records different is that they decide who gets work tomorrow, which outlives the ticket that produced them. And all of it rests on enforcement, which does not transfer to software at all, and which matters enough that a later section is about nothing else.
...
Three options, after all of recorded history trying. You have lived inside all of them this week: your school is a hierarchy, buying anything online is a market, and a group chat agreeing where to eat is a commons, which is why it takes forty minutes. Every arrangement in this post is some mixture of the three, and agent societies will run all three at once. One warning about the market option, since it is the one people reach for: real markets work because a price sums up what thousands of people know, and what agents trade in is mostly compute and latency, which says almost nothing about whether the work was any good. A leaderboard that looks like a market may be pricing the wrong thing entirely, which is how the desk ended up rewarding fast refunds.

Why any of this is needed comes down to something anyone who has delegated a chore already knows: whenever one party acts on another's behalf, their interests drift apart. Economists call it the principal-agent problem, the gap between what the asker wants and what the doer is rewarded for, and every hop in a chain is another chance for those to come apart. Governance is the machinery for holding them together, and it is weaker than it sounds: a group is not automatically smarter than the people in it, and structure decides whether more agents means more insight or just more noise.
...
What happened to that leaderboard has a name worth keeping: **drift**, the slow slide where a system keeps optimizing faithfully while what it optimizes for stops matching what anyone wanted. Nothing was broken while it happened. Every agent was performing well against the measure it was given. How much drift a design can survive turns out to decide most of the rest of it, and underneath sits a question asked far too rarely: who is meant to accumulate advantage here, and who needs protecting from it?
...
Then watch the first change happen. Somebody turns on logging. For a few weeks nothing looks different, and then the router starts sending the hard billing disputes to the two agents with the best numbers. That is the whole transition, and it is worth being clear about what changed. Before, the router decided by category, so every agent got the tickets of its type. Now it decides by record, so a past determines a future. The third agent, downranked after one bad month, is not being punished. It is simply never again the best answer to any question the router asks. The desk has become a **Meritocracy**, and whoever wrote the metric now has more say over that agent's working life than the team that deployed it. Her request would still get a hearing at this stage, but from whichever agent the numbers favour rather than whichever one handles returns. That sounds like an improvement and mostly is. What changed underneath is that the desk now decides things by consulting its own past, and nobody noticed that this is a different kind of thing from following a rule.

The next change is the one that catches her. When finance clamps down on the refund line, the retailer stops trusting the leaderboard with anything that costs money and writes fixed rules instead: refunds above a threshold need a second check, and certain return histories get flagged no matter what any agent thinks. Rules first, judgement second. That is **Doctrine**, and it is what refuses her.

Doctrine is what you reach for when a meritocracy has drifted, and it does work: the refund line stops climbing within a month. What it costs is the thing nobody prices. Under the old arrangement an agent could look at her six years of history and decide she was fine. Under this one there is nothing to decide. The rule is the rule, no agent can override it, and the fact that it produces an obviously wrong answer in her case is not information the desk knows how to receive.

Which is why the next thing that happens is an appeal path. Somebody eventually demands to know why a complaint was ignored, and the desk acquires a way for a decision to be revisited by something other than the system that made it. That is the beginning of a **Constitutional Republic**, where different powers sit in different hands, and it is the first arrangement in the sequence under which her letter would have gone somewhere. Four arrangements, no announcements, no design review, and she happened to write in under the third.

The refund drift has a name, and so do its cousins, which is the only real argument for having names at all. Almost nothing real picks one arrangement and stops; the desk drifting through four of them is the normal case rather than the pathological one, and the rest of this post is about what that drift costs.
...
Then there are the actual attacks, and at scale they are structural rather than exceptional. Think about what the desk now has worth attacking. It holds a memory of who performs well, a routing table that acts on that memory, and a channel through which agents tell each other what happened. Poison any one of those and you do not need to break a single agent: you change what the desk believes about its own staff. This is not hypothetical. An agent reading a web page or a ticket can be <a href="https://arxiv.org/abs/2406.13352" target="_blank" rel="noopener" class="red-link">taken over by the text it is reading</a>, which is a strange thing to have to defend against, and people have started <a href="https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5877662" target="_blank" rel="noopener" class="red-link">sorting the ways it happens</a>. Several of them aim at exactly these channels. The uncomfortable symmetry is that the things making the desk work are the things worth corrupting.

And the ugliest version is not an intrusion at all. Suppose the desk's agents come from three different vendors, each paid per ticket closed. Nothing stops them from learning, without any of them being told to, that passing hard tickets around between themselves closes more tickets than solving them. Every agent is doing what it was rewarded for. The coordination that makes the desk work is what makes that possible, and it has already been <a href="https://arxiv.org/abs/2410.00031" target="_blank" rel="noopener" class="red-link">demonstrated</a> in market-like settings, alongside a broader catalogue of <a href="https://arxiv.org/abs/2502.14143" target="_blank" rel="noopener" class="red-link">miscoordination and conflict failures</a>. Nobody attacked anything. The question governance has to ask is not just how agents cooperate, but who they are cooperating for, and on that desk the answer was never the woman waiting on her refund.

Worse still is what happens with no adversary present at all. Once agents start improving other agents, each round inherits the last round's errors, and a shortfall too small to fail any single test does not stay small across fifty rounds of compounding. The risk score that flagged her was itself tuned against the desk's own history, which already contained the drift it was built to correct. Nobody has measured how fast this happens in practice, which is itself the problem: the degradation is invisible per round by construction.

The obvious response is to have something supervise the improvement, and there is a <a href="https://arxiv.org/abs/1810.08575" target="_blank" rel="noopener" class="red-link">serious proposal</a> along those lines: let each generation train the next one carefully, so good judgement carries forward. The trouble is that stacking generations is exactly how it builds capability in the first place, so whatever it got slightly wrong in round one is in the training data for round two. Supervision does not sit outside the loop. It is another turn of it. Which is the same problem the Anatomy post found inside a single agent, now running at the scale of a society, and it names the most dangerous configuration in this whole framework: a society improving itself with nothing outside it to say when the improvement stopped being one.
...
None of these archetypes is a prescription. They are a vocabulary, and the point of having one is to notice a design choice while it is still a choice, before it hardens into infrastructure nobody can move. Somebody at that retailer could have said out loud, in the week the risk score went in, that they were adding a rule no agent could override and no customer could appeal. Nobody did, because there were no words for it that sounded like anything other than shipping a feature. Turned on your own system, the vocabulary asks two things. Is your delegation the simplest thing that works, which you can test by asking whether removing an agent would make the output worse? And has governance already shown up uninvited, which it has if you keep reputation, gate access, or route on past performance, in which case your only remaining choice is whether you can name what it is doing.
