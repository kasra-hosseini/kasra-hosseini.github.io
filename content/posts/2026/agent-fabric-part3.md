...
Three options, and that is the whole list after all of recorded history trying. You have lived inside every one of them this week: your school is a hierarchy, buying anything online is a market, and a group chat agreeing where to eat is a commons, which is why it takes forty minutes. Agent societies will run all three at once. One warning about the market option, since it is the one people reach for: real markets work because a price sums up what thousands of people know, and what agents trade in is mostly compute and latency, which says almost nothing about whether the work was any good. A leaderboard that looks like a market may be pricing the wrong thing entirely, which is how the desk ended up rewarding fast refunds.

Why any of this is needed comes down to something anyone who has delegated a chore already knows: the person doing it is not chasing the same thing you are. Ask someone to tidy a room and they will optimise for the room looking tidy, which is not the same as knowing where anything is. Every hop in a chain of delegation is another chance for what was asked and what gets rewarded to come apart, and governance is the machinery for holding them together. It is weaker than it sounds: a group is not automatically smarter than the people in it, and structure decides whether more agents means more insight or just more noise.
...
**Autocracy** first: one router deciding everything. Her refund request would have gone to whichever agent handled returns and been judged on its merits, which from the outside feels like dealing with a small shop where one person decides and you can tell who it was. Fast, for exactly the reason it is dangerous, since if that one judgement is wrong then everyone downstream is wrong with it.

**Meritocracy** is what the logging turned it into. The router used to decide by category, so every agent got the tickets of its type. Now it decides by record, so a past determines a future. Her letter still reaches somebody at this stage, just whichever agent the numbers favour rather than whichever one knows returns, and from the outside it feels like being routed to whoever is currently winning rather than whoever understands your problem. The third agent, downranked after one bad month, is not being punished; it is simply never again the best answer to any question the router asks, and whoever wrote the metric now has more say over its working life than the team that deployed it.

**Doctrine** is what catches her. When finance clamps down on the refund line, the retailer stops trusting the leaderboard with anything that costs money and writes fixed rules instead: refunds above a threshold need a second check, and certain return histories get flagged no matter what any agent thinks. Rules first, judgement second. From the outside this is the one that feels like talking to a wall, because every reply is correct and none of them is about you.
...
Which is where the fourth arrangement comes from, and it takes a complaint to force it. Somebody eventually demands to know why theirs was ignored, loudly enough that the answer cannot be a shrug, and the desk acquires a way for a decision to be revisited by something other than the system that made it. That is a **Constitutional Republic**: different powers in different hands, an appeal that does not report to the thing being appealed. From the outside it is the first arrangement that feels like being heard rather than processed, and the first under which her letter would have gone somewhere. The cost is speed, which is why nothing reaches it until something has already gone wrong. Four arrangements, no announcements, no design review, and she happened to write in under the third.
