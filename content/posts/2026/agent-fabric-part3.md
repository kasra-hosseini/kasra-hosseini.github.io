...
Step back from the desk and there are only three ways anyone has ever got a group to coordinate. Somebody can be in charge, which is a hierarchy. People can trade until prices sort it out, which is a market. Or the people who depend on a shared thing can run it together, with neither a boss nor a price, which is a commons, and it works far better in practice than it sounds like it should. You have lived inside all three this week: your school is the first, buying anything online is the second, and a group chat agreeing where to eat is the third, which is why it takes forty minutes. Agent societies will run all three at once.
...
Which means none of these arrangements is quite an institution in the usual sense. They behave like one while the actual enforcing is quietly subcontracted to whoever owns the machines. Call something a market when its discovery layer belongs to a single platform and you have a franchise wearing market clothing. So choosing a governance structure without naming who enforces it is half a design, and the half you skipped gets decided by somebody who has never heard of your project.
...
Send the same letter in at four different moments and watch where it lands. That is the whole difference between these arrangements, and the desk passed through all four without anybody filing a change request.
...
Which of them a team ends up with is mostly decided by the situation rather than chosen. Stable, well-understood work rewards the efficient arrangements; novel work rewards the flexible ones; and drift that has already cost something pushes toward hard rules and mandatory verification whether anyone wanted the overhead or not. The moment more than one organization is involved, a single centre stops being available at all. But the better instinct is to stop shopping for one archetype, because strong systems separate functions instead of choosing between them: something fast to execute, something bounding what it may do, something watching for drift, and somewhere to appeal. The question is which powers should be split so that no single failure captures everything.
...
One more failure needs no adversary and no bad metric at all. Agents are starting to improve other agents, and every generation inherits the last one's mistakes. The obvious fix is to have each generation train the next one carefully, except that stacking generations is how the thing builds capability in the first place, so the supervision is another turn of the loop rather than a check on it. The risk score that flagged her was tuned against the desk's own history, which already contained the drift it was built to correct. That is the most dangerous configuration in this whole framework: a society improving itself with nothing outside it to say when the improvement stopped being one.
...
Which is why asking how secure it is turns out to be the wrong question. The thing being attacked is never one agent. On that desk it would be the scoreboard, the routing table that reads it, and the memory joining them, so a poisoned score does not break anything: it just sends the sensitive work toward whichever agent arranged to look good. Three components each behaving correctly, and the failure living in the gaps between them. Which arrangement you chose decides where those gaps are, because capturing the router while the desk is still an autocracy gets you every ticket, while under hard rules the individual agents stop being worth attacking and whoever writes the rules becomes the whole target. Ask which attack you actually expect, not how strong the walls are.
