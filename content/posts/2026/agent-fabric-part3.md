...
Why any of this is needed comes down to a problem old enough to have a name. Whenever one party acts on another's behalf their interests can drift apart, which economists call the <a href="https://en.wikipedia.org/wiki/Principal%E2%80%93agent_problem" target="_blank" rel="noopener" class="red-link">principal-agent problem</a>, and every delegation hop is one more chance for what was asked and what gets done to come apart. Governance is the machinery for holding those together across depth, time, and organizational boundaries, and it is worth being sober about how well it works: the most robust finding in the study of human groups is that they are not automatically smarter than their members (<a href="https://doi.org/10.1126/science.1193147" target="_blank" rel="noopener" class="red-link">Woolley et al. 2010</a>). More agents can mean more insight or just more noise. The structure decides which.
...
Economists spent a century working out when each one wins (<a href="https://en.wikipedia.org/wiki/The_Nature_of_the_Firm" target="_blank" rel="noopener" class="red-link">Coase</a> on firms, <a href="https://en.wikipedia.org/wiki/The_Use_of_Knowledge_in_Society" target="_blank" rel="noopener" class="red-link">Hayek</a> on prices, <a href="https://en.wikipedia.org/wiki/Elinor_Ostrom#Design_principles_for_Common_Pool_Resource_(CPR)_institution" target="_blank" rel="noopener" class="red-link">Ostrom</a> on commons), and the borrowing is loosest for markets: Hayek's point was that prices carry information nobody could have centralized, while the compute-and-latency "prices" agents trade in carry far less.

Some of this gets designed on purpose. The rest <a href="https://en.wikipedia.org/wiki/Self-organization" target="_blank" rel="noopener" class="red-link">self-organizes</a>, norms hardening out of repeated interaction while nobody watches, which is not a new discovery: complexity researchers spent decades showing that interacting agents under resource pressure reliably generate structures nobody designed (<a href="https://en.wikipedia.org/wiki/John_Henry_Holland" target="_blank" rel="noopener" class="red-link">Holland</a> 1995, <a href="https://en.wikipedia.org/wiki/Stuart_Kauffman" target="_blank" rel="noopener" class="red-link">Kauffman</a> 1993). Those findings come from biology and physics, though, so whether language-model agents behave the same way remains open.
...
Picture what that looks like when it goes wrong. A dozen companies build a federation: shared protocol, negotiated rules, each firm governing its own agents, nobody in charge of everybody. On paper it is the most legitimate arrangement in this post. Every one of them runs on the same cloud provider, because everyone does. Then the provider triples the price of the inference tier the protocol depends on, and the federation discovers that its constitution was a pricing page all along. Nobody voted. Nobody was overruled. The rules simply stopped being affordable, which turns out to be the same thing as stopping.
...
Which means a society needs ways to be argued with. Hirschman's *Exit, Voice, and Loyalty* (1970) named how members respond when an institution decays, and the first two transfer directly. The third does not: Loyalty in his model is the bond that keeps people inside complaining rather than leaving, and software ecosystems have little of it. What they have instead is Fork, a constructive exit that carries the shared artifact forward rather than abandoning it.
...
The other set cannot be engineered away at all, because each needs somebody to make a judgement call. Should reputation travel between contexts, when a track record earned under one set of conditions may mean nothing under another? How do you supervise a thousand agents, given that reviewing their decisions is arithmetically impossible and the alternative is a set of signals that might preserve real oversight or merely its appearance? And underneath both, the question this post has now deferred twice: agents cannot consent to being governed, so what makes authority over something that cannot agree to it legitimate at all, and is that even a coherent thing to ask?
