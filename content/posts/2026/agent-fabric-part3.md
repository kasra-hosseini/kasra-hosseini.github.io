...
Governance grows out of what gets remembered, and in one year that desk will pass through four recognisable ways of being governed, without anyone at the retailer deciding on a single one of them. One agent decides, which is an **autocracy**. Then the scoreboard decides, which is a **meritocracy**. Then fixed rules decide, which is **doctrine**. Then, if anyone shouts loudly enough, the rules get checked by something that did not write them, which is a **constitutional republic**.
...
The desk did not choose fixed rules either. Drift that has already cost money pushes hard toward them, whether or not anyone wanted the overhead, and that is how the risk score arrived. Going fast, staying inside limits, noticing drift, and being able to reverse a decision are four different jobs. The desk was doing all four with one number, and that number was the one that refused her.
