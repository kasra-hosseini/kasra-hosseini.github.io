...
Neither persistence nor memory alone would do it, though. A cache persists and governs nothing; what counts is authority over how work gets handed out, outliving the task that produced it. And all of it rests on enforcement, which does not transfer to software at all, and which matters enough that a later section is about nothing else.
...
One word will keep recurring, so it is worth naming. **Drift** is the slow slide where a system keeps optimizing faithfully while what it optimizes for stops matching what anyone wanted. The leaderboard rewarding fast refunds was drift, and notice that nothing was broken while it happened: every agent was performing well against the measure it was given. How much of that you can survive turns out to determine most of your design. Underneath sits a question asked far too rarely: who is meant to accumulate advantage here, and who needs protecting from it?
...
The next change is the one that catches her. When finance clamps down on the refund line, the retailer stops trusting the leaderboard to decide anything that costs money and writes hard rules instead: refunds above a threshold need a second check, and certain return histories get flagged regardless of what any agent thinks. That is **Doctrine**, a constitution the router itself cannot override, and it is the thing that refuses her. Doctrine is what you reach for when a meritocracy has drifted, and it works, in the sense that the refund line stops climbing. It just cannot hear her.
