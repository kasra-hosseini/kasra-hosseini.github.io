...
That is the bet this post is making. Agent systems will fail through bad institutions rather than bad answers, and those failures will not look like one agent returning something wrong. They will look like a market that quietly rewards collusion, a scoreboard that games itself, a chain of delegation nobody can audit, and rules that turn out to belong to somebody else. What follows is how to see that coming.
...
The router is the whole transition in miniature, and the line it crossed falls in a precise place. A router that forwards traffic is playing the game; a router that keeps score and decides who may touch what is writing the rules the game has to follow. Economists studying human institutions draw exactly that line: an institution is the rules of the game, not the players and not the moves. Persistence alone is not enough, since a cache persists and governs nothing. What counts is standing authority over how resources get handed out, authority outliving any single task.

That definition rests on one more thing, though, which is enforcement, and enforcement does not transfer to agents at all. It matters enough that a later section is about nothing else.

Before any of the names, watch one of these fail. The desk's leaderboard is working: the two best agents get the hard billing disputes, everyone is happy, resolution times drop. What nobody notices is that the metric is resolution time, and the fastest way to resolve a hard dispute is to approve the refund. So the two most trusted agents on the desk gradually become the two most generous, the leaderboard rewards them for it, and the desk's standing hierarchy is now optimising against the company that built it. No agent lied. No attacker was involved. The scoreboard simply measured the wrong thing and everyone competed honestly. (Twenty-two of these arrangements are catalogued in a companion [field guide](/posts/2026/governance-archetypes-field-guide/), with what each is good at and how each fails; nothing here depends on it.)

Why any of this is needed comes down to something anyone who has ever delegated a chore already knows: whenever one party acts on another's behalf, their interests drift apart. Economists have a name for it, the <a href="https://en.wikipedia.org/wiki/Principal%E2%80%93agent_problem" target="_blank" rel="noopener" class="red-link">principal-agent problem</a>, and every hop in a delegation chain is one more chance for what was asked and what gets done to come apart. Governance is the machinery for holding those together across depth, time, and organizational boundaries, and it is worth being sober about how well that works. Decades of <a href="https://doi.org/10.1126/science.1193147" target="_blank" rel="noopener" class="red-link">research on human groups</a> found something deflating: a group is not automatically smarter than the people in it. More agents can mean more insight or just more noise, and the structure is what decides which.
...
You have lived inside all three this week: your school is a hierarchy, buying anything online is a market, and a group chat agreeing where to eat is a commons, which is why it takes forty minutes. The desk has already tried two of them. It started as a hierarchy, one router deciding, and then the leaderboard turned it partly into a market, where agents effectively compete for the good tickets on the strength of their record.
...
Take the pricing-page problem seriously for a moment, because it has a consequence people skip.

If enforcement always lives one layer below your design, then none of these arrangements is an institution in the sense the word usually carries. They are platform-enforced coordination regimes that behave like institutions, with the enforcement quietly subcontracted to whoever owns the substrate. A market whose discovery layer belongs to a single platform was never really a market, only a franchise in market clothing. Which means choosing a governance structure without naming who enforces it is half a design, and the half you skipped is usually decided by somebody who has never heard of your project.
...
The refund drift has a name, and so do its cousins, which is the only real argument for having names at all. Figure 1 lays them out by where the authority sits. Almost nothing real picks one and stops, and the desk drifting through four of them is the normal case rather than the pathological one.
...
Whatever you pick, you are also picking winners, and on the desk the winners were the two fastest refunders. Generalise that and the pattern is uncomfortable. An autocracy serves whoever runs the hub. Markets reward reputation, which compounds, so whoever arrived first keeps winning. A colony quietly favours early entrants whose habits calcify into everyone else's defaults. And even meritocracy, the one that sounds fairest, hands real power to whoever writes the benchmark.
...
Worse still is what happens with no adversary present at all. Once agents start improving other agents, small misalignments survive into the next generation and compound. Take a deliberately crude illustration, ours and not a measurement, which cheats by treating alignment as a single number that multiplies cleanly: at 99.9 percent preserved per round, fifty rounds still leave you around 95 percent, but at 99 percent, a gap invisible in any single test, fifty rounds leave you near 60. The details of that arithmetic are wrong. The direction is not.

Even iterated amplification and distillation (Christiano et al. 2018), proposed as an alignment method, carries the exposure, because anything that builds capability across rounds inherits each round's errors.
