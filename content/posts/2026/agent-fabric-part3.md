...
The router is the whole transition in miniature, and the line it crossed falls in a precise place. A router that forwards traffic is playing the game; a router that keeps score and decides who may touch what is writing the rules the game has to follow. Economists studying human institutions draw exactly that line: an institution is the rules of the game, not the players and not the moves. Two words get used a lot below, so it is worth separating them now. The **institution** is the thing that exists, the standing rule about who may do what. **Governance** is what it does, the ongoing business of applying that rule and settling who gets to change it. The desk acquired an institution the month it started keeping score. It has been governed ever since, by nobody in particular. Persistence alone would not do it, since a cache persists and governs nothing; what counts is authority over how resources get handed out, outliving any single task.
