...
If governance shows up whether or not you invited it, the only real question is what kind you end up with. This post is a field guide to the answers: twenty-two recognizable ways an agent society can be run, what each one is good at, how each one fails, and who quietly benefits from each. Then the harder question underneath all of them, which is who actually enforces any of it.
