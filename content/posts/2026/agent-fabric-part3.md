...
**Meritocracy** next, and you can watch it arrive. Somebody turns on logging. For a few weeks nothing looks different, and then the router starts sending the hard billing disputes to the two agents with the best numbers. Before, it decided by category, so every agent got the tickets of its type. Now it decides by record, so a past determines a future. The third agent, downranked after one bad month, is not being punished; it is simply never again the best answer to any question the router asks. The cost is that whoever wrote the metric now has more say over that agent's working life than the team that deployed it.
...
**Constitutional Republic** is where it goes next. Somebody eventually demands to know why a complaint was ignored, and the desk acquires a way for a decision to be revisited by something other than the system that made it, which means different powers now sit in different hands. It is the first arrangement in the sequence under which her letter would have gone somewhere. The cost is speed, and the reason it usually arrives last is that nothing else forces it. Four arrangements, no announcements, no design review, and she happened to write in under the third.
