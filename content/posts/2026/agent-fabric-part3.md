...
What is new is that somebody owns the conditions. An ant colony's chemistry is not up for negotiation, but an agent society runs on protocols, reward structures, and starting configurations that a company controls and can change on a Tuesday. You cannot redesign a pheromone. You can absolutely redesign a reputation score.
...
Then there are the actual attacks, and at scale they are structural rather than exceptional. Think about what the desk now has worth attacking. It holds a memory of who performs well, a routing table that acts on that memory, and a channel through which agents tell each other what happened. Poison any one of those and you do not need to break a single agent: you change what the desk believes about its own staff. This is not hypothetical. Tool-using agents can be <a href="https://arxiv.org/abs/2406.13352" target="_blank" rel="noopener" class="red-link">steered by data they were only supposed to read</a>, and the environment-side versions have been <a href="https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5877662" target="_blank" rel="noopener" class="red-link">catalogued into six classes</a>, several aimed at exactly those channels. The uncomfortable symmetry is that the things making the desk work are the things worth corrupting.
...
The obvious response is to have something supervise the improvement, and there is a <a href="https://arxiv.org/abs/1810.08575" target="_blank" rel="noopener" class="red-link">serious proposal</a> along those lines: let each generation train the next one carefully, so good judgement carries forward. The trouble is that stacking generations is exactly how it builds capability in the first place, so whatever it got slightly wrong in round one is in the training data for round two. Supervision does not sit outside the loop. It is another turn of it.

This is what iterative optimization does when nothing outside the loop corrects it, which makes it the same problem the Anatomy post found inside a single agent, now running at the scale of a society. Governance is that correction, and it names the most dangerous configuration in this whole framework: a society improving itself with nothing outside it to say when the improvement stopped being one.
