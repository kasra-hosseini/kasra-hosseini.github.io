...
The same logic runs one level up, with no adversary and no bad metric involved. Agents are starting to improve other agents, and each generation inherits the mistakes of the one before it. The obvious answer is to have every generation train the next one carefully, except that stacking generations is how the thing gains capability in the first place, so that supervision is another turn of the same loop rather than a check on it. The risk score that flagged her account was tuned against the desk's own history, which already contained the drift it was built to correct. That is the most dangerous configuration in this whole framework: a society improving itself with nothing outside it to say when the improvement stopped being one.
