...
Here is something worth sitting with: for all of recorded history, humans have found only three basic ways to make a group coordinate, and every archetype in this post is some mixture of them. The first is putting somebody in charge. That is a hierarchy, and it is fast precisely because it concentrates every risk in one place. The alternative is to stop deciding centrally at all and let people trade until prices sort it out, which is a market, flexible right up until somebody works out how to game the scoring. Or, hardest and most surprising of the three, the people who depend on a shared resource can govern it together without either a boss or a price, which is a commons: remarkably durable in practice, and agonizingly slow to decide anything. Economists spent the twentieth century working out why each exists and when each wins, and the specific arguments repay attention (<a href="https://en.wikipedia.org/wiki/The_Nature_of_the_Firm" target="_blank" rel="noopener" class="red-link">Coase</a> on firms, <a href="https://en.wikipedia.org/wiki/The_Use_of_Knowledge_in_Society" target="_blank" rel="noopener" class="red-link">Hayek</a> on prices, <a href="https://en.wikipedia.org/wiki/Elinor_Ostrom#Design_principles_for_Common_Pool_Resource_(CPR)_institution" target="_blank" rel="noopener" class="red-link">Ostrom</a> on commons; the Related Work section maps each onto agent systems). Those one-line characterizations are ours, not theirs. And the borrowing is loosest for the market case, because Hayek's real point was that prices carry information nobody could have centralized, whereas the compute-and-latency "prices" agents trade in carry much less of it. Expect agent societies to run all three at once.
...
Twenty-two archetypes follow, some already running in production and some only plausible given where things are heading. Comparing them comes down to four questions asked over and over. How much useful work do you get for what it costs, which the cards call *efficiency*. How well does the structure stop goals from quietly wandering, which they call *drift resistance*. Whether a human can afterwards reconstruct what happened and why, which is *legibility*. And whether the participants will treat a decision as binding, which the cards call *legitimacy* and which needs a warning attached, because that word is borrowed and it does not fit.

In political philosophy, legitimacy is about the governed consenting to be governed, and agents cannot consent to anything. So read it here in the narrow engineering sense: will the participants, and the humans answerable for them, treat this structure's rulings as binding and worth defending? That is a real property with real consequences, and it is not the property philosophers argue about. Open Problems comes back to what gets lost in the swap.
...
Now the harder question: what happens when nobody is in charge on purpose? These next arrangements distribute authority instead of locating it, which sounds obviously better until you notice what it costs. Deliberation is slow. Delegation chains get long and hard to trace. Norms that emerge from nothing in particular can drift toward anything at all.
...
Which brings us to the thing that undercuts everything above, and it is worth slowing down for. Every archetype in this post is a claim about who decides. But a decision only means something if it can be made to stick, and no agent can make anything stick on its own.
