...
<p style="color: #555;">What differs from biological or economic self-organization is speed (agent societies can reorganize in hours, not generations), bandwidth (agents can share full operational context, not lossy summaries), transparency (agent interactions can be logged and audited), and designability (humans can shape the conditions under which self-organization occurs). The bandwidth point deserves emphasis. <a href="https://inverseprobability.com/atomic-human" target="_blank" rel="noopener" class="red-link">Lawrence (2024)</a> draws out the gap between human communication and machine communication, which is not a close contest: our outbound bandwidth is a handful of bits per second, and machines exchange context orders of magnitude faster. The exact figure depends on how you count, and any of the plausible counts leaves the gap enormous. Human societies self-organize under severe communication constraints; agents do not face the same bottleneck. The coordination tax that limits human organizations is structurally lower for agents, which means agent societies can form, restructure, and dissolve faster than any human institution. The bottom-up path is self-organization: order arises from local interactions without a central planner directing the outcome. The top-down path is the deliberate engineering of structures and conditions. Both can produce similar societies; they differ in origin, not necessarily in form. Both are present in complex adaptive systems.</p>
</details>

The Loom Hypothesis does not predict universal connection. It predicts that agents will coordinate where shared context is valuable and coordination costs are absorbable. A spam classifier scoring one message at a time is the clear negative case: it holds no state worth sharing, it gains nothing from another agent's history, and connecting it adds an attack surface and no accuracy. Bounded, stateless, single-purpose work stays isolated and should.
