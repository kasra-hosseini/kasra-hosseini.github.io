...
The two layers are not rivals; governance structures are built out of delegation patterns. An autocracy runs on chains and evaluators, a market on auctions, a federation on routers and relays across a boundary. A few names appear in both lists, which is worth pausing on, because Mission Command as a delegation pattern means telling one agent the intent behind one task, while Mission Command as governance means a society-wide commitment that this is how all tasks get communicated and a rule about who may change it. Same mechanism, different scope, and the scope is where the authority lives.

## Governance: How Agent Societies Are Ruled

It is worth being exact about when machinery turns into something worth calling an institution. The economist <a href="https://en.wikipedia.org/wiki/Douglass_North" target="_blank" rel="noopener" class="red-link">Douglass North</a> described institutions as the rules of the game, as distinct from the players or the moves. By that test, a system routing tasks through a pipeline is executing a design, while a system that keeps reputation scores, enforces who may touch what, and decides which agents get promoted or benched is writing the rules the design has to follow. Persistence alone is not the line, since a cache persists and governs nothing. The line is standing authority over how resources get allocated, holding beyond any single task. (North's account also rests on enforcement, which does not transfer cleanly to agents, and that turns out to matter enough to come back to at the end of this section.)

Underneath all of it sits a problem old enough to have a name: whenever one party acts on another's behalf, their interests can diverge. Economists call this the <a href="https://en.wikipedia.org/wiki/Principal%E2%80%93agent_problem" target="_blank" rel="noopener" class="red-link">principal-agent problem</a>, and it is the reason intent leaks as delegation deepens. Every hop is one more chance for what was asked and what gets done to come apart, which is exactly the compounding drift described earlier. Governance is the machinery for keeping those two things aligned across depth, time, and organizational boundaries.
...
Humans have only ever found three basic ways to get a group to coordinate, and every archetype here is some mixture of them. You can put somebody in charge, which is a hierarchy: fast, but it concentrates the risk in one place. You can let people trade and let the price sort it out, which is a market: flexible, and gameable by anyone who figures out the scoring. Or the people who depend on a shared resource can govern it together, which is a commons: remarkably durable, and slow to decide anything. Economists spent the twentieth century working out why each one exists and when it wins, and the specific arguments are worth knowing if you want the depth (<a href="https://en.wikipedia.org/wiki/The_Nature_of_the_Firm" target="_blank" rel="noopener" class="red-link">Coase</a> on firms, <a href="https://en.wikipedia.org/wiki/The_Use_of_Knowledge_in_Society" target="_blank" rel="noopener" class="red-link">Hayek</a> on prices, <a href="https://en.wikipedia.org/wiki/Elinor_Ostrom#Design_principles_for_Common_Pool_Resource_(CPR)_institution" target="_blank" rel="noopener" class="red-link">Ostrom</a> on commons; the Related Work section maps each onto agent systems). Those one-line characterizations are ours, not theirs. And the borrowing is loosest for the market case, because Hayek's real point was that prices carry information nobody could have centralized, whereas the compute-and-latency "prices" agents trade in carry much less of it. Expect agent societies to run all three at once.
...
Twenty-two archetypes follow, some already running in production and some only plausible given where things are heading. Comparing them means asking the same four questions each time. How much useful work do you get for what it costs? How well does it stop goals from quietly wandering? Will the participants treat its decisions as binding? And can a human afterwards reconstruct what happened and why? That third one needs a caveat, because the natural word for it is legitimacy and that word carries more weight than it can bear here: agents cannot consent to being governed the way people can, so all it means in this post is fitness for accountability and buy-in. The Open Problems section comes back to what gets lost in that substitution.

Treat the ratings themselves as hunches rather than measurements. They assume a stable task distribution under a single deployer, and a different environment can reorder them. What they are good for is seeing the shape of a trade, that a Colony spends legibility to buy flexibility, or that Doctrine buys resistance to drift and pays in adaptability.
...
Every one of those choices is a trade, and the trades are stubborn. Efficiency costs you resistance to drift. Legitimacy costs you speed. Legibility costs you flexibility. No system escapes to a corner where it gets both; it just settles somewhere along each line depending on what is being built, what the builder can afford to risk, and who is trusted with what.
...
The tractable ones are still genuinely difficult. Start with identity, which sounds trivial until you notice that every authentication scheme ever built assumes the thing being identified persists, whereas an agent can be cloned, merged, or rolled back to last Tuesday before lunch. Provenance breaks in a related way: when an agent works inside several societies at once, tracing which society's rules actually produced a given output means following a chain that snaps at every boundary it crosses. And societies under stress change their own rules, the way democracies reach for emergency powers and markets simply stop trading. Deciding what should trigger such a switch is one problem. Making sure the emergency mode ever ends is the harder one.

The other set cannot be engineered away, because they require someone to make a judgement call. Whether reputation should travel between contexts is really a question about whether a track record earned under one set of conditions means anything under different ones, and assuming it does is how you get a well-reviewed agent failing in a situation nothing prepared it for. Human oversight runs into arithmetic: nobody can meaningfully supervise a thousand agents by reviewing decisions, so the real question is what minimal set of signals preserves genuine oversight rather than its appearance. And underneath everything sits the question this post has deferred twice. Agents cannot consent to being governed. What makes authority over something that cannot agree to it legitimate at all, and is that even a coherent thing to ask?
