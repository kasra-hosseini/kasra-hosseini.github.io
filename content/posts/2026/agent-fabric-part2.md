...
Delegation is the easy half: how a task flows through a group of agents, who does what, who checks the result, and who pays the compute and latency bill for each step. A data-processing pipeline that forgets everything between runs is pure delegation, and it will never be anything else. Governance is the harder half, and it is about authority rather than flow: who gets to decide, on what basis, and how that decision can be challenged. Part 1 called a multi-agent system a society once it develops shared context, routing that depends on history, and learning that crosses between agents. Governance is the structure that grows on top of those three things, whether or not anyone planted it.
...
One piece of vocabulary before we start. Everything below is built from the same unit: a model extended with memory that survives between calls, tools that act on the world, and enough planning to break a goal into steps. A bare model maps input to output; this thing loops, remembers, and acts. The companion post, [The Anatomy of an Agent](/posts/2026/anatomy-of-an-agent/), takes that unit apart and shows why one of them alone hits a ceiling. It is the atom. What follows is the chemistry.

## The Division of Labour

Every multi-agent system has to answer one question first: how does the work get split? With a handful of agents, an orchestrator handing out assignments is enough. Add more and the shape of the delegation stops being an implementation detail and becomes the architecture itself. Push it far enough, to the scales Part 1 imagines, and the question stops being about splitting work at all and becomes about who has standing to decide, which is the second half of this post.

### Delegation Archetypes

There are forty-three recognizable patterns below, grouped into nine families with one meta-pattern that operates on the structure itself. Nobody needs to memorize them. The grouping exists so you can find the three or four that describe whatever you are actually building, and the boundaries leak in places (Speculative Execution is half quality-check, half market).
...
**Hierarchical patterns** break a task apart and reassemble the pieces: authority flows down, results come back up. They carry a quiet danger. If every hop preserves only most of what the original requester meant, the losses multiply with depth, so a deep tree can be busily and confidently working on something nobody asked for. Tomašev et al. (2026) call the version of this that matters for blame an "accountability vacuum." The practical lesson is that the shallowest tree that works is also the safest one.
...
**Human-agent interface patterns** decide where a person sits in the chain: what stops and waits for approval, who is watching, and what is alarming enough to interrupt someone. All of them are working around the same hard limit, which is that human attention does not scale with agent count. You cannot review a thousand decisions a day, so each of these patterns is a different bet about which decisions are worth your attention.
...
No real system runs on a single pattern, and the useful ones are recipes rather than ingredients. Chain a router to an escalation ladder and an evaluator loop and you get a quality gate, whose whole philosophy is to route cheaply, escalate rarely, and verify always. Fan a task out and make the copies vote before merging, and you get reliability that does not depend on trusting any single agent. Keep a pipeline's stages fixed but auction off each slot, and the shape stays designed while the staffing is settled by competition. Figure 3 animates all three.
...
All of this costs money, and the money shapes the architecture more than any design document does. Every extra level of delegation multiplies both cost and latency, and compute, memory, bandwidth, and API quota are all finite. Model performance does improve smoothly as you scale compute and data, but on a curve with diminishing returns (<a href="https://arxiv.org/abs/2001.08361" target="_blank" rel="noopener" class="red-link">Kaplan et al. 2020</a>), and the same logic seems to apply to spending inference inside a delegation tree, though that version is less well characterized. What it adds up to is selection pressure. A tree that answers in a hundred tokens survives a budget review that kills the one needing ten thousand for the same answer, so over time the population drifts toward efficiency. The winner is not the deepest tree but the shallowest one that reliably produces a good enough answer, and in high-volume work an agent delivering 95% of the quality at 10% of the cost tends to beat one delivering everything at full price. (For the resource ecology behind this, see [Part 1, Figure 5](/posts/2026/agent-fabric-part1/#the-resource-ecology).)
...
Delegation tells you how work flows through a tree. Who sits at each node? Today's frontier models are generalists. Generalism, however, is expensive, and that cost creates steady pressure toward **specialization** (how far it goes is the open question the box above flags). Layered systems where specialized models feed outputs into aggregators can collectively surpass frontier models (<a href="https://arxiv.org/abs/2406.04692" target="_blank" rel="noopener" class="red-link">Mixture-of-Agents</a>), and for many narrow tasks, especially classification and routing, the cost gap between a frontier model and a small specialist can be large enough to dominate the architecture. The advantage goes not to the largest model, but to the **best-composed ensemble**.
...
Recall the distinction: delegation is how work flows through agents; governance is who gets to decide, on what authority, and how that authority is challenged. A Chain that processes a code review is delegation. The rules that determine which code model is trusted with security-sensitive refactors, and who can change those rules, are governance. Multi-agent systems use delegation. Societies add governance on top.
...
The coding agent is not unique. The transition from architecture to institution follows the same structural path across domains. Operational data accumulates, that data shapes routing, and routing-with-memory becomes authority. What makes this authority rather than mere optimization is that the preference outlives any single task and starts to constrain future decisions: a load balancer forgets last week's latencies, but a fleet that will not route to an unproven agent until it clears a learned threshold is enforcing a standing rule no one wrote. Three more examples make the pattern concrete. They are illustrations of where current trajectories point, not case studies of deployed systems; the transition they describe depends on persistence infrastructure that is still arriving.
...
Recall that governance is the institutional structure that forms around a society's three properties (shared context, interaction-dependent routing, cross-agent learning). What makes it an institution, rather than mere machinery, is worth stating precisely. In <a href="https://en.wikipedia.org/wiki/Douglass_North" target="_blank" rel="noopener" class="red-link">Douglass North</a>'s framing (*Institutions, Institutional Change and Economic Performance*, 1990), institutions are "the rules of the game." A multi-agent system that routes tasks through a Pipeline is executing a design. A system that maintains reputation scores, enforces access policies, and decides which agents get promoted or demoted is **setting the rules of that design**. The threshold is not persistence alone (a cache is persistent but not governance). It is **authority relationships that constrain how resources are allocated and how decisions are made**, independently of any single task. (One part of North's account, enforcement, does not transfer cleanly to agents; we return to what that means for the "institution" label at the end of this section.)

Every delegation is a <a href="https://en.wikipedia.org/wiki/Principal%E2%80%93agent_problem" target="_blank" rel="noopener" class="red-link">principal-agent relationship</a>: the delegator (principal) wants one thing; the delegate (agent) may want another. The central question of governance is how to make the agent's interests converge with the principal's across delegation depth, time, and organizational boundaries. This is why the multiplicative drift sketched above is hard to avoid without governance: each hop is one more principal-agent gap where intent can leak.

Once the Loom Hypothesis produces a society (see [Part 1](/posts/2026/agent-fabric-part1/)), the next question is how it is governed. Governance is what keeps a society's shared context, routing, and learning tractable rather than chaotic.

Different governance structures are different strategies for managing the tension between the two observations from [Part 1](/posts/2026/agent-fabric-part1/). The first observation demands utility. Agents persist only while useful. The second constrains it. Resources are finite, but agent populations can grow without bound. Every governance model is a bet on how to balance these forces. A consistent finding from collective intelligence research, observed in human groups, is that groups are not automatically smarter than individuals (<a href="https://doi.org/10.1126/science.1193147" target="_blank" rel="noopener" class="red-link">Woolley et al. 2010</a>); group performance depends on composition, communication, incentives, and process. Whether the same holds for agent collectives is an open question, but the design lesson carries: more agents can mean more insight, or merely more noise.

Most governance archetypes can be understood as mixtures of three older coordination forms, whose logics were studied by <a href="https://en.wikipedia.org/wiki/The_Nature_of_the_Firm" target="_blank" rel="noopener" class="red-link">Coase</a> (why firms exist), <a href="https://en.wikipedia.org/wiki/The_Use_of_Knowledge_in_Society" target="_blank" rel="noopener" class="red-link">Hayek</a> (how prices distribute knowledge), and <a href="https://en.wikipedia.org/wiki/Elinor_Ostrom#Design_principles_for_Common_Pool_Resource_(CPR)_institution" target="_blank" rel="noopener" class="red-link">Ostrom</a> (how commons self-govern): **hierarchy**, **market**, and **commons** (see Related Work expandable for how each thinker's contribution applies to agent systems). As a rough shorthand, hierarchy is fast but concentrates risk, markets are flexible but gameable, and commons are resilient but slow to decide; these are our own summary characterizations, not claims those authors made. The analogy is loosest for Hayek: his argument is specifically that market prices transmit dispersed knowledge, and the compute-and-latency "prices" agents trade in carry far less of that signal than currency does. Agent societies will likely combine all three forms.

Some governance is **designed**. A company deploys an orchestrator with explicit roles. Some is **emergent**. Agents <a href="https://en.wikipedia.org/wiki/Self-organization" target="_blank" rel="noopener" class="red-link">self-organize</a> through repeated interaction, and norms crystallize without anyone planning them. This is not a wholly new idea. The complexity science community has shown across decades of research that systems of interacting agents under resource constraints tend to produce emergent coordination structures (<a href="https://en.wikipedia.org/wiki/John_Henry_Holland" target="_blank" rel="noopener" class="red-link">Holland</a>, *Hidden Order*, 1995; <a href="https://en.wikipedia.org/wiki/Stuart_Kauffman" target="_blank" rel="noopener" class="red-link">Kauffman</a>, *The Origins of Order*, 1993; see also Part 1's discussion of self-organization). Those results come from biological, physical, and simple computational systems; whether LLM-based agents self-organize the same way is an empirical question, not a settled one. What agent societies add is that the conditions for self-organization can themselves be designed. Unlike ant colonies or traffic systems, agent societies run on protocols, reward structures, and initial configurations that a deployer controls. You cannot redesign pheromone chemistry, but you can redesign a reputation mechanism or a routing policy.

A society's circumstances largely predict what it becomes. Predictable work rewards hierarchy, while novel work rewards markets and colonies. Drift that would be catastrophic forces you toward rules and verification; drift you can absorb leaves room for looser arrangements. One deployer permits a center, several organizations rule it out. And running underneath those is a question that gets asked far less often: who is supposed to accumulate advantage here, and who needs protecting from it. Those constraints define a space, and the archetypes below are points scattered inside it.

Twenty-two archetypes follow, some already running in production and some only plausible given where things are heading. Each card rates them on four things: how much useful work they get per unit of cost, how well they stop goals from wandering, whether the participants will accept a decision as binding, and whether a human can later reconstruct what happened and why. A note on the third one, since the word carries weight it should not carry here: agents cannot consent to governance the way people can, so legitimacy in this post means fitness for accountability and buy-in, not the normative thing political philosophers argue about. Open Problems returns to that gap.

The ratings themselves are ordinal hunches, not measurements, and they assume a stable task distribution under a single deployer. Change the environment and the ordering can shift. Their only real use is comparative, to make visible that Colony trades away legibility to buy flexibility, or that Doctrine buys drift resistance and pays for it in adaptability.
...
Whatever you pick, you are also picking winners, and it is worth being honest about that. Autocracy serves the hub operator. Markets reward whoever already has reputation, which compounds. Franchise favors the platform over everyone building on it. Colony favors early entrants, whose habits calcify into everyone else's defaults. Meritocracy favors whoever writes the benchmark. "Which structure is most efficient" is only half the question; the other half is who accumulates the advantage, and at whose expense.

And there is a harder catch underneath all of it. Whoever controls the compute, the API access, and the protocol definitions sets the real terms, regardless of the archetype on the diagram. A Federation running entirely on one provider's infrastructure is a federation until that provider changes its pricing. A Market whose discovery layer belongs to a single platform is a Franchise wearing market clothing. This is also where the comparison to human institutions finally strains: North's institutions bind because breaking them carries costs nobody can escape, whereas a judicial agent's ruling binds an executive agent only because the runtime chooses to enforce it. That is true of every archetype here, not just one. So these are not institutions in North's full sense. They are platform-enforced coordination regimes that behave like institutions, with the enforcement quietly delegated upward to whoever owns the substrate. Which means an archetype chosen without naming its enforcer is only half a design, and governance design and platform design are the same problem.
...
Archetypes on their own are just parts. What they look like assembled is more interesting, so the tables below work through concrete situations across three horizons: things running today, things plausible in a few years, and things that are frankly speculative. None of it is meant as prediction. The point is narrower and more useful, which is that a coding assistant and a warehouse robot fleet have both already made governance choices, and neither one filed any paperwork about it. Naming the choice is how you get to make it on purpose.
...
Voice is the cheapest of the three and the easiest to forget to build. It means an agent can escalate a disagreement, flag something that looks wrong, or ask for human arbitration without being punished for the trouble. That is what keeps governance responsive, because problems surface while they are still small. Take it away and failures accumulate silently until something breaks all at once.

Exit is the threat that makes voice worth listening to. An agent that keeps drawing low-quality work can move to a competing orchestrator; a deployer who stops trusting a society can pull its agents out entirely. That possibility disciplines whoever is in charge, though only when a real alternative exists, which is precisely what platform concentration erodes.

Fork is the last resort, and the one software understands best. When a governance disagreement genuinely cannot be resolved, forking beats forcing consensus, because different domains really do need incompatible rules. A security team may need every connection verified while an exploratory research team suffocates under exactly that regime. Splitting lets both be right.
...
So if you are building one of these systems, the vocabulary earns its keep on three questions. Start with whether your delegation is the simplest thing that works: the common failure is adding agents for their own sake, and the test is brutally simple, which is whether removing one would actually make the output worse. Then ask whether governance has already shown up uninvited. If your system tracks reputation, gates access, or routes on past performance, it is governing, and the only real question is whether you can name what it does. Finally, look up the characteristic weakness of whatever you have built, because each archetype fails in its own signature way. Autocracies get captured at the center. Markets collude. Colonies drift. Find out which one is yours before it finds you.
...
If it holds, though, it lands differently on different people. If you build these systems, the lesson is that governance is not a feature you schedule for later, because it arrives on its own schedule, and designing it deliberately is far cheaper than discovering it after it has become load-bearing. If you regulate them, the lesson is that auditing individual model outputs is looking at the wrong object; the thing to examine is the society, meaning its delegation structure, its drift resistance, and who its power distribution quietly favors. And if you research them, the uncomfortable lesson is that multi-agent safety does not reduce to single-agent alignment. Collusion, norm drift, reputation gaming, and governance capture are population-level phenomena, and the tools that describe them look a lot more like institutional economics than like loss functions.
