...
Naming the arrangements is the easy part. What a catalogue cannot tell you is what happens when they nest inside each other, which is where the interesting behaviour and nearly all of the cost lives. Start with the simplest kind of nesting, **agents that spawn other agents**. Your research agent needs a paper summarised, so it hires something to fetch the paper and something else to compress it, and each of those may hire further. A tree forms, and you never see it.
...
Push that further and the tree crosses hardware. The frontier model in some datacentre reasons about your week; a mid-sized model on your laptop drafts replies; something tiny on the phone itself decides whether a notification is worth showing you at all. Each layer hands work down to something cheaper and dumber, which is the only reason any of this is affordable. It also means the thing deciding whether to interrupt you is the least capable component in the chain.
