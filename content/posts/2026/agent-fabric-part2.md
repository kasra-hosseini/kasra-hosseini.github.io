...
Those trees stay short, though not by good design. Every extra agent costs money and adds delay, so budgets push toward the fewest layers that do the job. That has a side effect nobody planned. A track record is just a stored number: this agent resolved 94 percent of what it was given, that one 71 percent. Fewer layers means fewer agents carrying such a number, and therefore fewer places where a number quietly starts deciding who gets the real work. Being cheap limits how much authority can pile up.
...
The same pressure decides who sits at each node. A narrow specialist is cheaper than a generalist and often better at its one job, so the advantage can end up belonging to a well-assembled team of small models rather than to the largest single one (<a href="https://arxiv.org/abs/2406.04692" target="_blank" rel="noopener" class="red-link">one such approach</a> has each model answer independently, then merges the results). How far that goes is genuinely unsettled. (Part 1's [resource ecology](/posts/2026/agent-fabric-part1/#the-resource-ecology) is the long version.)
