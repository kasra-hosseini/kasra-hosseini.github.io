...
And each of those agents is already doing the same thing internally. Ask the research one for a summary and it quietly hires something to fetch the paper and something else to compress it, and that one may hire further. You asked one question and started a chain of command that nobody can see whole, because each agent knows only who called it and who it called. Delegation like that is old news and already everywhere. What is new is what happens when these arrangements start remembering how it went.
...
The cost everyone budgets for is compute: every extra agent is another model call, another wait, another chance for the brief to blur, which is why nobody builds twenty layers deep when three will do. The cost nobody budgets for is the one Priya ran into. Ask the router to send a hard billing dispute to the third agent and it will comply, once, and then go back to the two it trusts, because nothing you said changed the record and the record is what it consults. Nobody granted it that power. Split work among agents, keep records of how it went, act on those records, and you have built an institution without filing any paperwork: preferences you did not choose, standing you did not assign, and a memory that outlives the task that created it.
