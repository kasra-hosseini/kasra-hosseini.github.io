...
None of this is speculative tooling. The frameworks that assign distinct roles and coordinate them already exist (<a href="https://arxiv.org/abs/2308.08155" target="_blank" rel="noopener" class="red-link">AutoGen</a>, <a href="https://arxiv.org/abs/2303.17760" target="_blank" rel="noopener" class="red-link">CAMEL</a>, <a href="https://arxiv.org/abs/2308.00352" target="_blank" rel="noopener" class="red-link">MetaGPT</a>), and production systems from the <a href="https://corporate.zalando.com/en/technology/more-personal-and-smarter-zalando-assistant-enhanced-capabilities-inspire-customers" target="_blank" rel="noopener" class="red-link">Zalando Assistant</a> to Microsoft's <a href="https://arxiv.org/abs/2411.04468" target="_blank" rel="noopener" class="red-link">Magentic-One</a> run orchestrators that plan, track progress, and re-plan when something fails. Worth noting how modest the real ones are, though: today's trees run two or three levels deep, not twenty. The version on your phone is shallower still.
...
Push that further and the tree crosses hardware. The frontier model in some datacentre reasons about your week, a mid-sized model on your laptop drafts replies, and something tiny on the phone decides whether a notification is worth showing you. Each layer hands work down to something cheaper, which is the only reason any of this is affordable. It also means the component with the most authority over your attention is the least capable thing in the chain.
...
The same decompose, dispatch, aggregate motion repeats at every level, which makes delegation look pleasingly fractal until you check the two ends. At the leaves it stops, because somebody has to do the irreducible work. At the root it stops too, because that is where a human's intent enters, which in your case means you. Everything in between is a chain of things guessing what you meant.

Which is why the best advice in the field is also the least exciting: start with one well-tuned model call and only reach for orchestration once the simple thing has demonstrably failed (Anthropic's <a href="https://www.anthropic.com/engineering/building-effective-agents" target="_blank" rel="noopener" class="red-link">field report</a> is blunt about this). Every agent you add is another place records accumulate and another candidate for standing you never assigned. One counterintuitive detail is worth keeping, though: the agent doing the coordinating does not have to be the strongest one in the system, and a small model that has learned how to delegate can direct much larger ones (see <a href="https://arxiv.org/abs/2512.04695" target="_blank" rel="noopener" class="red-link">TRINITY</a>).

All of this costs money, and the money ends up shaping the architecture more than any design document does. Every extra level of delegation multiplies both cost and latency, which becomes a brutally simple selection pressure: the arrangement that answers in a hundred tokens survives the budget review that kills the one needing ten thousand for the same answer. Which is why your phone does not consult a frontier model about whether to show you a notification. The winner is never the deepest tree, it is the shallowest one that reliably produces a good enough answer. And notice what the cheap option buys: the shallowest arrangement that works is also the one with the fewest places for standing to accumulate, which makes frugality and accountability accidental allies.
