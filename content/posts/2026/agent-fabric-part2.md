...
A society's circumstances largely predict what it becomes. Predictable work rewards hierarchy, while novel work rewards markets and colonies. Then there is drift, which is the thing this whole field quietly worries about most: the slow slide where a system keeps optimizing faithfully while what it optimizes for stops matching what anyone wanted. If drift would be catastrophic in your setting, you end up reaching for hard rules and mandatory verification. If you can absorb some, looser arrangements stay available. Add one more constraint, which is that a single owner permits a center while several organizations rule one out, and underneath all of it a question that gets asked far too rarely: who is meant to accumulate advantage here, and who needs protecting from it? Those constraints define a space, and the archetypes below are points scattered inside it.
