...
And each of those agents is already doing the same thing internally. Ask the research one for a summary and it quietly hires something to fetch the paper and something else to compress it, and that one may hire further. Three layers is plenty, because every extra agent is another wait and another chance for the brief to blur, and even at three you asked one question and started a chain of command nobody can see whole, since each agent knows only who called it and who it called. None of which is new. What is new is that these chains have started remembering how each call went.
