...
*This post is part of The Agent Fabric blog series. [Part 1](/posts/2026/agent-fabric-part1/) established two observations. First, agents persist only while useful to a deployer. Second, resources are finite, while agent populations can grow much faster than the resources available to run them (and these bounds shift as societies form and reshape their environment). Together these produce the Loom Hypothesis: a pressure toward connected, coordinated configurations.*

*This post picks up where that pressure leads. It asks how agents divide work, and how governance structures form around them. One thread from Part 1 runs throughout: societies have two origins. Some are built top-down, when a company or team designs the structure; others form bottom-up, when individuals assemble agents from what is available. As Part 1 argued, the two paths differ in origin, not necessarily in form: the same delegation and governance patterns can arise through either. What the origin does shape is whether a structure was chosen deliberately or crystallized from interaction, a distinction that recurs throughout this post.*
...
**Delegation has an economics.** Every level of delegation multiplies cost and latency. Agents consume compute, memory, bandwidth, and API quotas, and these resources are finite. Neural scaling laws show that model performance improves smoothly as you scale compute, data, and parameters, but the returns diminish on a log-log curve (<a href="https://arxiv.org/abs/2001.08361" target="_blank" rel="noopener" class="red-link">Kaplan et al. 2020</a>). The same diminishing-returns logic applies to inference cost within delegation trees, though the empirical relationship at the tree level is less well characterized. At scale, this creates selection pressure. A delegation tree that solves a task in 100 tokens survives budget constraints that kill one requiring 10,000 tokens for the same result. Over time, the population shifts toward efficiency. The future may not belong to the deepest tree, but to the **shallowest tree that reliably produces a good enough answer**. In many high-volume domains, an agent that achieves, say, 95% of the quality at 10% of the cost will tend to outcompete one that achieves 100% at 100%. (For the resource ecology that makes this possible, see [Part 1, Figure 5](/posts/2026/agent-fabric-part1/#the-resource-ecology).)
...
**What determines governance type?** A society's situation predicts which archetype it develops. *Task predictability* (stable tasks favor hierarchy; novel tasks favor markets or colonies). *Drift tolerance* (catastrophic drift demands Doctrine or Zero-Trust; acceptable drift permits Colony or Market). *Organizational boundary* (single deployer enables centralized archetypes; multiple organizations demand Federation). *Power distribution* (who should accumulate advantage, and who should be protected from it). Together these define the design space; the 22 archetypes below are points within it.
...
**Enforcement, and what these archetypes really are.** North's institutions work because violating them carries costs no single actor can escape: legal sanction, ostracism, credible exclusion. Agent societies have no direct analogue. As the Constitutional Republic card notes, a judicial agent's ruling binds an executive agent only because the runtime enforces it, and that is true of every archetype here, not just one. Every governance structure in this taxonomy is ultimately enforced by the platform (the compute, the API layer, the protocol definitions). This does not empty the taxonomy, but it sharpens what it is a taxonomy *of*: these are not institutions in North's full sense but platform-enforced coordination regimes that exhibit the structural properties of institutions (authority relationships, allocation rules, drift resistance) while delegating enforcement upward to whoever controls the infrastructure. The practical consequence is that choosing an archetype without specifying who enforces it is an incomplete design. Governance design and platform design are the same problem.
