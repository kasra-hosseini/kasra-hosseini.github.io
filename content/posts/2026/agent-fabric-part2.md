...
By March the third agent has stopped getting hard tickets altogether. In April the engineer who built the routing layer sorts the log and sees the column of zeroes. Priya knows she did not write that. She goes looking for the rule that did, and finds nothing: not in the settings, not in the policy docs, and not in the memory of anyone who could have changed it. There is no rule. There is only a pattern that has been running long enough to look like one.
...
By October the third agent is retired for underperformance. Now only two agents have any record on hard billing disputes, and in November one of them starts approving refunds it should not. Somewhere in that month a customer is told her fragile-item return does not qualify, by an agent that has never been especially good at fragile items, and there is nothing left to compare that answer against: the one agent that might have looked different was retired for never being given the chance.
